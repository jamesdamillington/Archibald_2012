...
def test_grid_initialization():
    model = FireModel(grid_size=10, mu=0.1, p_spread=0.2, rho=0.5)
    grid = model.grid
    assert grid.shape == (10, 10)
    assert np.all(np.isin(grid, [NON_FLAMMABLE, UNBURNT]))

def test_initial_flammable_fraction():
    model = FireModel(grid_size=20, mu=0.1, p_spread=0.2, rho=0.7)
    frac = model.calculate_initial_flammable_fraction()
    assert 0.0 <= frac <= 1.0

def test_ignite_changes_some_cells():
    model = FireModel(grid_size=10, mu=1.0, p_spread=0.2, rho=1.0)
...
def test_spread_fire_only_to_adjacent():
    model = FireModel(grid_size=5, mu=0.0, p_spread=1.0, rho=1.0)
    model.grid[:] = UNBURNT
...
def test_update_burnt():
    model = FireModel(grid_size=5, mu=0.0, p_spread=0.0, rho=1.0)
    model.grid[:] = UNBURNT
...
def test_initial_num_flammable_clusters():
    model = FireModel(grid_size=10, mu=0.1, p_spread=0.2, rho=1.0)
...
def test_burned_fraction_and_num_fires():
    model = FireModel(grid_size=10, mu=1.0, p_spread=0.0, rho=1.0)
